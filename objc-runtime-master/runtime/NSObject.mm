...
+ (BOOL)isKindOfClass:(Class)cls {
    //for 循环执行规则
    //第一步：表达式1，即 Class tcls = object_getClass((id)self)，然后再表达式二，判断tcls是否存在，然后执行表达式4
    //判断 tcls == cls是否相等，然后最后再执行表达式三、tcls = tcls ->superclass
    //第二次循环，判断表达式二是否成立，如果成立，执行表达式四，如果不相等则继续表达式三
    //第N次循环继续下去
    
    //do while 循环 与 while 循环 的区别就在于do while 会至少运行一次，while 可能一次都不运行
    //swich循环
    //break 与 return 的区别
    
    for (Class tcls = object_getClass((id)self); tcls; tcls = tcls->superclass) {
        //实例的对象的isa 指向的是 class对象的isa
        //class 对象的isa 指向是 superclass 的isa
        //superclass的isa 指向的是 meatclass 的isa
        // metaclass 的isa 指向的是基类的isa
